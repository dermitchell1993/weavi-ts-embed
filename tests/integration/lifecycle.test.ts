...
const TEST_TIMEOUT_LIFECYCLE = 120000; // 120s: DB startup + health check + operations + shutdown
const TEST_TIMEOUT_MULTI_CYCLE = 360000; // 360s: 3 full cycles with cleanup delays
const TEST_TIMEOUT_RAPID_CYCLE = 240000; // 240s: 2 rapid cycles with minimal delays
 ...
 * @param timeoutMs Timeout for the HTTP request (default: 2000ms)
 */
function isHealthy(host: string, port: number, timeoutMs = 2000): Promise<boolean> {
  return new Promise((resolve) => {
    const options = {
      hostname: host,
      port,
...
describe('Lifecycle Integration Tests', () => {
  ...
  afterEach(async () => {
    ...
    // Separate stop() and wait logic to ensure cleanup even if stop() throws
    if (client?.embedded?.pid) {
      const pid = client.embedded.pid;

      // Try to stop gracefully, but don't let exceptions prevent cleanup
      try {
      ...
      // Always wait for process to stop, regardless of stop() success
      try {
  ...
  describe('Basic Lifecycle', () => {
    ...
    it(
      ...
      async () => {
        ...
        const firstPid = client.embedded.pid;
        trackedPids.push(firstPid);

        // Wait for first instance to be fully started
        ...
        // Assert: Current implementation logs "already listening" but doesn't throw
        // This test verifies the client can be created without crashing
        // NOTE: This is current behavior - ideally should throw an error (see implementation line 132-134)
        const client2 = await weaviate.client(options2);

        // Should return the same client instance or same PID
        expect(
          client2.embedded.pid === firstPid,
          'Should reuse existing instance when port is occupied'
        ).toBe(true);
      },
      TEST_TIMEOUT_LIFECYCLE
    );
    ...
    it(
      'should start process with custom environment variables',
      async () => {
        // Arrange
        const customEnv = {
  ...
  describe('Graceful Shutdown (SIGTERM)', () => {
    it(
      'should stop gracefully with SIGTERM and release resources',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should handle stop() being called multiple times safely',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should complete in-flight operations before shutting down',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
  ...
  describe('SIGKILL Fallback', () => {
    it(
      'should force kill process with SIGKILL when process is hung',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should detect and handle already-terminated process',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
  ...
  describe('Multiple Start/Stop Cycles', () => {
    it(
      'should handle 3 consecutive start/stop cycles without resource leaks',
      async () => {
        const cycles = 3;
    ...
    it(
      ...
      TEST_TIMEOUT_RAPID_CYCLE
    );

    it(
      'should maintain data persistence across restart cycles',
      async () => {
  ...
  describe('Unexpected Termination & Cleanup', () => {
    it(
      'should detect when process terminates unexpectedly',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should clean up resources when parent process exits',
      async () => {
    ...
    it(
      'should not leave zombie processes after termination',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
  ...
  describe('Error Handling & Edge Cases', () => {
    it(
      'should handle stop() called before process fully starts',
      async () => {
    ...
    it(
      'should validate that stop() with invalid PID fails gracefully',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should handle port conflict detection correctly',
      async () => {
        // Arrange: Start first instance
        const options = new EmbeddedOptions({
          port: testPort,
  ...
  describe('Health Polling', () => {
    it(
      'should successfully poll health endpoint until ready',
      async () => {
        // Arrange
        const options = new EmbeddedOptions({
          port: testPort,
    ...
    it(
      'should timeout if health endpoint never responds',
      async () => {
